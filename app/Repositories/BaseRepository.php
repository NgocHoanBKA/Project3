    ...
    public function allQuery($search = [], $skip = null, $limit = null, $orders = [])
    {
        ...
        if (count($search)) {
            foreach ($search as $key => $value) {
                if (in_array($key, $this->getFieldsSearchable())) {
                    ...
                    $method_or = 'filterOr' . Str::studly($key);
                    if (method_exists($this, $method)) {
                        $this->{$method}($value);
                    } else if (method_exists($this, $method_or)) {
                        $this->{$method_or}($search);
        ...
        if (is_array($orders) and count($orders)) {
            foreach ($orders as $orderBy => $orderDir) {
                $method = 'order' . Str::studly($orderBy);
                if (method_exists($this, $method)) {
                    $this->{$method}($orderBy, $orderDir);
                } else {
